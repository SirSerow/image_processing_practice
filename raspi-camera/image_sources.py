...
def capture_images(picam2, directory):
    ...
    # 各ストリームから画像をキャプチャします
    main_image = os.path.join(directory, "main_image.jpg")
    lores_image = os.path.join(directory, "lores_image.jpg")
    raw_image = os.path.join(directory, "raw_image.data")

    # Capture the main and lores images
    # main および lores 画像をキャプチャします
    picam2.capture_file(main_image, name="main")
    picam2.capture_file(lores_image, name="lores")
    ...
    print(f"Main image saved to {main_image}")
    print(f"Lores image saved to {lores_image}")
    print(f"Raw image data saved to {raw_image}")
